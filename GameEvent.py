...
class GameEvent:
    ...
    def set_player_in_check(self):
        ...
        try:
            ...
            player.king.check = (
                False  # If no enemy piece puts king in check, set to False
            )
            return False

        except Exception as e:
            print(
                f"Error in set_player_in_check: {e}"
        ...
        if self.mined.remaining == 0:
            if isinstance(self.mined, Quarry):
                ...
                if self.engine.get_occupying(self.mined.row, self.mined.col):
                    self.piece_removed = self.engine.get_occupying(
                        self.mined.row, self.mined.col
                    )
...
class Teleport(RitualEvent):
    def __init__(self, engine, acting_tile, action_tile):
        ...
        if self.action_tile_has_effective_trap(
