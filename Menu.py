...
class KingMenu(Menu):
    ...
    def mouse_move(self):
        ...
        is_within_menu_y = (
            self.menu_position_y <= mouse_y <= self.menu_position_y + self.menu_height
        )
    ...
    def left_click(self):
        pos = pygame.mouse.get_pos()
        if pos[0] in range(
            self.menu_position_x, self.menu_position_x + self.menu_width
        ):
            if pos[1] in range(
                self.menu_position_y, self.menu_position_y + self.menu_height
            ):
...
class QueenMenu(Menu):
    def __init__(self, row, col, win, engine):
        ...
        self.color = Constant.turn_to_color[
            self.engine.turn
        ]  # Color based on the current turn
        ...
        self.cost_display_x = center_x - (
            self.cost_text_surface.get_width() // 2
        )  # Center the cost text
        self.gold_icon_display_x = (
            self.cost_display_x + self.cost_text_surface.get_width() + horizontal_buffer
        )  # Place gold icon next to cost

        # Set vertical positioning (center both vertically in the menu)
        self.cost_display_y = (
            self.menu_height // 2 - self.cost_text_surface.get_height() // 2
        )  # Vertically center the cost text
        self.gold_icon_display_y = (
            self.cost_display_y
        )  # Align the gold icon with the cost text vertically

        # Debug: print the positions to ensure they are correct
        print(f"Cost Text Position: ({self.cost_display_x}, {self.cost_display_y})")
        print(
            f"Gold Icon Position: ({self.gold_icon_display_x}, {self.gold_icon_display_y})"
        )
    ...
    def draw(self):
        ...
        self.menu.blit(
            self.resource_icon, (self.gold_icon_display_x, self.gold_icon_display_y)
        )  # Draw gold icon
        self.menu.blit(
            self.cost_text_surface, (self.cost_display_x, self.cost_display_y)
        )  # Draw cost text
