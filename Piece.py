...
class King(Piece):
    ...
    def spawn_squares(self, engine):
        ...
        for direction in range(len(self.spawn_directions)):
            d = self.spawn_directions[direction]
            r = self.row - d[0]
            c = self.col - d[1]
...
class Pawn(Piece):
    ...
    def spawn_squares(self, engine):
        ...
        # RIGHT
        c2 = c + 1
        if engine.has_no_resource(r, c2) and engine.has_none_occupying(r, c2):
            spawn_squares.append((r, c2))
        ...
        # DOWN
        r2 = r + 1
        if engine.has_no_resource(r2, c) and engine.has_none_occupying(r2, c):
            spawn_squares.append((r2, c))
    ...
    def mining_squares(self, engine):
        ...
        c2 = c + 1
        if engine.has_mineable_resource(r, c2):
            if engine.get_occupying(r, c2):
                if engine.get_occupying_color(r, c2) is not self.color:
                    pass
                elif engine.get_occupying_color(r, c2) is self.color:
                    mining_squares.append((r, c2))
            elif engine.has_none_occupying(r, c2):
                mining_squares.append((r, c2))
        # LEFT
        c2 = c - 1
        if engine.has_mineable_resource(r, c2):
        ...
        # DOWN
        r2 = r + 1
        if engine.has_mineable_resource(r2, c):
            if engine.get_occupying(r2, c):
                if engine.get_occupying_color(r2, c) is not self.color:
                    pass
                elif engine.get_occupying_color(r2, c) is self.color:
                    mining_squares.append((r2, c))
            elif engine.has_none_occupying(r2, c):
                mining_squares.append((r2, c))
        # UP
        r2 = r - 1
        if engine.has_mineable_resource(r2, c):
...
class Monk(Piece):
    ...
    def valid_moves(self, engine):
        ...
        for move in valid_moves:
            if engine.can_be_occupied(move[0], move[1]):
