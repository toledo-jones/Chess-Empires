...
class Trades:
    ...
    def get_conversions(self):
        ...
        for resource in self.resources:
            conversions[resource] = (
                random.uniform(self.give_lower_bound, self.give_upper_bound),
                random.uniform(self.receive_lower_bound, self.give_upper_bound),
            )
        return conversions

    def get_receive_conversion(self, give_amount, receive_resource):
        receive_rate = self.engine.trade_conversions[self.engine.turn_count_actual][
            receive_resource
        ][1]
    ...
    def get_give_conversion(self, give_resource, player):
        ...
        give_rate = self.engine.trade_conversions[self.engine.turn_count_actual][
            give_resource
        ][0]
