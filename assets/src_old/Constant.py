...
DEBUG_RITUALS = False
POP_UPS_ON = True
from Settings import *  # This is imported here to override the lines above when debugging is active
STARTING_PRAYER = 0
STARTING_WOOD = 0
STARTING_GOLD = 0
STARTING_STONE = 0
...
for i in range(19):
    pray.append(i)
for i in range(12):
    print(f"adding {i} to {generate_resources}")
...
def load_music():
    ...
    pygame.mixer.music.load(os.path.join("../music/ambience", filename + '.wav'))
...
def load_images():
    for image in images:
        ...
        IMAGES[image] = pygame.transform.scale(pygame.image.load(os.path.join("../images", image + ".png")),
                                               (scale[0], scale[1])).convert_alpha()
    for resource in resources:
        scale = RESOURCES_IMAGE_MODIFY[resource]['SCALE']
        RESOURCES[resource] = pygame.transform.scale(
            pygame.image.load(os.path.join("../resources", resource + ".png")),
            (scale[0], scale[1])).convert_alpha()
    for menu_icon in menu_icons:
        scale = MENU_ICONS_IMAGE_MODIFY[menu_icon]['SCALE']
        MENU_ICONS[menu_icon] = pygame.transform.scale(
            pygame.image.load(os.path.join("../menu_icons", menu_icon + ".png")),
            (scale[0], scale[1])).convert_alpha()

    for piece in w_pieces:
        scale = PIECE_IMAGE_MODIFY[piece_color_to_type(piece)]['SCALE']
        W_PIECES[piece] = pygame.transform.scale(pygame.image.load(os.path.join("../pieces", piece + ".png")),
                                                 (scale[0], scale[1])).convert_alpha()
    for piece in w_buildings:
        scale = PIECE_IMAGE_MODIFY[piece_color_to_type(piece)]['SCALE']
        W_BUILDINGS[piece] = pygame.transform.scale(pygame.image.load(os.path.join("../pieces", piece + ".png")),
                                                    (scale[0], scale[1])).convert_alpha()
    for piece in b_pieces:
        scale = PIECE_IMAGE_MODIFY[piece_color_to_type(piece)]['SCALE']
        B_PIECES[piece] = pygame.transform.scale(pygame.image.load(os.path.join("../pieces", piece + ".png")),
                                                 (scale[0], scale[1])).convert_alpha()
    for piece in b_buildings:
        scale = PIECE_IMAGE_MODIFY[piece_color_to_type(piece)]['SCALE']
        B_BUILDINGS[piece] = pygame.transform.scale(pygame.image.load(os.path.join("../pieces", piece + ".png")),
                                                    (scale[0], scale[1])).convert_alpha()
    ...
    for ritual in w_prayer_rituals:
        ...
        PRAYER_RITUALS[ritual] = pygame.transform.scale(
            pygame.image.load(os.path.join("../prayer_rituals", ritual + ".png")),
            (scale[0], scale[1])).convert_alpha()
