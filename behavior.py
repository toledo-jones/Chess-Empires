...
class Behavior:
    def __init__(self):
        ...
        self.can_perform_move = {
            "pray": self.can_pray,
            "steal": self.can_steal,
            "mine": self.can_mine,
            "spawn": self.can_spawn,
            "move": self.can_move,
            "capture": self.can_capture,
            "persuade": self.can_persuade,
        }
        self.fulfill_move_parameters = {
            "pray": self.pray,
            "steal": self.steal,
            "mine": self.mine,
            "spawn": self.spawn,
...
class MaterialCounter(Behavior):
    def __init__(self):
        ...
        self.resource_values = {"gold": 1.3, "log": 0.8, "stone": 1}
    ...
    def select_starting_square(self, engine):
        if self.starting_square is None:
            ...
            if not engine.is_legal_starting_square(
                self.starting_square[0], self.starting_square[1]
            ):
                self.starting_square = None
                return self.select_starting_square(engine)
            ...
            for piece in possible_moves:
                for move_kind in possible_moves[piece]:
                    for move in possible_moves[piece][move_kind]:
                        ...
                        event = self.fulfill_move_parameters[move_kind](
                        ...
                        if change_turn_event:
                            ...
                            current_evaluation = self.search(engine, depth - 1, False)[
                                1
                            ]
                        else:
    ...
    def print_diagnostics(
        self, turn, possible_moves, piece, move_kind, current_evaluation
    ):
