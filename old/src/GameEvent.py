    ...
    def complete(self):
        ...
        if self.engine.turn_count_actual == len(self.engine.piece_stealing_offsets) - 1:
            self.engine.piece_stealing_offsets.append(
                self.engine.generate_stealing_offsets(Constant.STEALING_KEY['pieces']))
        if self.engine.turn_count_actual == len(self.engine.building_stealing_offsets) - 1:
            self.engine.building_stealing_offsets.append(
                self.engine.generate_stealing_offsets(Constant.STEALING_KEY['buildings']))
...
class Swap(RitualEvent):
    ...
    def complete(self):
        ...
        # Here I have to figure out if each pieces is swapped to a portal square
        if self.first_trap:
