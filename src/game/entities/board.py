    ...
    def set_tile_pattern(self, tree_tile_sprites):
        for col in range(self.cols):
            for row in range(self.rows):
                texture_path = random.choice(list(tree_tile_sprites.keys()))
                tree_texture_surface = pygame.transform.scale(tree_tile_sprites[texture_path], (self.sq_size+2, self.sq_size+2))
        ...
        for col in range(self.cols):
            for row in range(self.rows):
                ...
                for item in to_be_rendered:
                    self.event_system.emit('draw sprite', item)
